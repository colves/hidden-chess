...
function init() {
    ...
    document.querySelectorAll('.promo-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            game.executeMove(p.fromR, p.fromC, p.toR, p.toC, false, type);
...
async function handleJoinGame() {
    ...
    try {
        ...
        alert(err.message || "Bağlantı kurulamadı. Kod yanlış olabilir.");
...
function goToSetupPhase() {
    ...
    document.getElementById('pieces-repository').style.display = 'flex';
    
    // Init drag drop
    dragDropManager.createInitialPieces(document.getElementById('pieces-repository'), peerConnection.isHost);
    game.startSetup();
...
function handleRematchRequest() {
    ...
    peerConnection.sendMessage('REMATCH_REQUEST', {});
...
function checkRematch() {
    if (iWantRematch && opponentWantsRematch) {
        ...
        document.querySelector('.instruction').innerText = "Taşlarınızı ilk iki satıra yerleştirin.";
        
        // Let host recreate the lobby phase
        if (peerConnection.isHost) {
            goToSetupPhase();
        }
    }
